function [A B C isConverged qual_hist C_max_hist B_max_hist A_max_hist corr_B_hist test_qual_hist time iter_cnt] = nonlinear_alpha_beta_CQBWQ(I, alpha, beta, maxIterCnt, eps, ...
    ...
    for currIter = 1:maxIterCnt
        if (alpha == 0 && beta == 0)
            ...
            F = (A * C) ./ (1 + B * C);
            B = B - (sum(bsxfun(@rdivide, C, 1 + B * C) .* log((I + eps_nnz) ./ (F + eps_nnz)), 2) + alpha_B  ./IQ .* B) ./ ...
                (sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* (log((F + eps_nnz) ./ (I + eps_nnz)) + 1), 2) + alpha_B  ./IQ);
            B = max(B, 0);
        elseif (alpha == 0)
            F = (A * C) ./ (1 + B * C);
            ...
            F = (A * C) ./ (1 + B * C);
            B = B + (alpha_B  ./IQ .* B + (1 ./ A) .* sum(power_my(F, beta + 1) .* log((F + eps_nnz) ./ (I + eps_nnz)), 2)) ./ ...
                (sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* power_my(F + eps_nnz, beta) .* (1 + (beta + 1) * log((F + eps_nnz) ./ (I + eps_nnz))), 2) + alpha_B  ./IQ);
            B = max(B, 0);
        elseif (beta == 0)  % OK
        ...
        elseif (alpha == -beta)     % OK
            ...
            % optimizing B
            F = (A * C) ./ (1 + B * C);
            B = B - ((1 / alpha) * sum(bsxfun(@rdivide, C, 1 + B * C) .* power_my(F, beta) .* (power_my(I + eps_nnz, alpha) - power_my(F, alpha)), 2) + alpha_B  ./IQ .* B) ./ ...
                ((1 / alpha) * sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* ...
                power_my(F, beta) .* ((alpha + beta + 1) * power_my(F, alpha) - (beta + 1) * (power_my(I + eps_nnz, alpha))), 2) + alpha_B  ./IQ);
...
%         end
        ...
        currQuality_reg = nmf_alpha_beta_divergence(I, Q, alpha, beta) + 0.5 * alpha_B *  sum(B .^ 2 ./ IQ) + 0.5 * alpha_C * sum(C .^ 2);
        ...
        fprintf('%d: %f\t%f\t%e\t%e\t%e\n', currIter, currQuality, (currQuality_reg-currQuality)/currQuality*100,  max(C_norm), max(B_norm), max(A_norm));
        %fprintf('%d: %e\n', currIter, C(912));
