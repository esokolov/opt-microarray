...
for currIter = 1:maxIterCnt
    if (alpha == 0 && beta == 0)
        F = (A * C) ./ (1 + B * C);
        A = exp((1 / size(I, 2)) * sum((log(I + eps_nnz) - log(bsxfun(@rdivide, C, 1 + B * C) + eps_nnz)) .* W, 2));
        A = max(A, 0);
        
        F = (A * C) ./ (1 + B * C);
        C = C + 0.5 * (sum(((1 ./ (eps_nnz + bsxfun(@plus, C, B * (C .^ 2)))) .* (log(I + eps_nnz) - log(F + eps_nnz))) .* W, 1) + alpha_C * C) ./ ...
            (sum(((1 ./ (eps_nnz + bsxfun(@times, C .^ 2, (1 + B * C) .^ 2))) .* ((2 * B * C + 1) .* log((I + eps_nnz) ./ (F + eps_nnz)) + 1)) .* W, 1) + alpha_C/2);
        C = max(C, 0);
        
        F = (A * C) ./ (1 + B * C);
        B = B - 0.5 * (sum((bsxfun(@rdivide, C, 1 + B * C) .* (log(I + eps_nnz) - log(F + eps_nnz))) .* W, 2) + alpha_B * B) ./ ...
            (sum((bsxfun(@times, C .^ 2, (1 + B * C) .^ 2) .* (log(F + eps_nnz) - log(I + eps_nnz) + 1)) .* W, 2) + alpha_B);
        B = max(B, 0);
    elseif (alpha == 0)
        F = (A * C) ./ (1 + B * C);
        A = A - 0.5 * ((1 ./ (A + eps_nnz)) .* sum((((F + eps_nnz) .^ beta) .* log((F + eps_nnz) ./ (I + eps_nnz))) .* W, 2)) ./ ...
            ((1 ./ (eps_nnz + A .^ 2)) .* sum((((F + eps_nnz) .^ beta) .* ((beta - 1) * log((F + eps_nnz) ./ (I + eps_nnz)) + 1)) .* W, 2));
        A = max(A, 0);
        
        F = (A * C) ./ (1 + B * C);
        C = C - 0.5 * (sum((((F + eps_nnz) .^ (beta + 1)) .* log((F + eps_nnz) ./ (I + eps_nnz)) .* (1 ./ (eps_nnz + A * (C .^ 2)))) .* W, 1) + alpha_C * C) ./ ...
            (sum(((1 ./ (eps_nnz + bsxfun(@times, C .^ 2, (1 + B * C) .^ 2))) .* ((F + eps_nnz) .^ beta) .* ...
            (1 - (2 * B * C - beta + 1) .* log((F + eps_nnz) ./ (I + eps_nnz)))) .* W, 1) + alpha_C/2);
        C = max(C, 0);
        
        F = (A * C) ./ (1 + B * C);
        B = B + 0.5 * (alpha_B * B + (1 ./ (A + eps_nnz)) .* sum((((F + eps_nnz) .^ (beta + 1)) .* log((F + eps_nnz) ./ (I + eps_nnz))) .* W, 2)) ./ ...
            (sum((bsxfun(@rdivide, C .^ 2, (1 + B * C) .^ 2) .* ((F + eps_nnz) .^ beta) .* ...
            ((beta + 1) * log((F + eps_nnz) ./ (I + eps_nnz)) + 1)) .* W, 2) + alpha_B/2);
        B = max(B, 0);
    elseif (beta == 0)
        A = (sum((I + eps_nnz) .^ alpha, 2) ./ sum(((eps_nnz + bsxfun(@rdivide, C, 1 + B * C)) .^ alpha) .* W, 2)) .^ (1 / alpha);
        A = max(A, 0);
        ...
        C = C - 0.5 * (sum(((((eps_nnz + (A * C) ./ (1 + B * C)) .^ alpha) - (eps_nnz + I) .^ alpha) ./ ...
            ...
            bsxfun(@times, C .^ 2, (1 + B * C) .^ 2)) .* W, 1) + alpha_C/2);
        C = max(C, 0);
        ...
        B = B - 0.5 * (sum((bsxfun(@times, C, (I + eps_nnz) .^ alpha - (eps_nnz + (A * C) ./ (1 + B * C)) .^ alpha) ./ ...
            ...
            ((1 + B * C) .^ 2)) .* W, 2) + alpha_B/2);
        B = max(B, 0);
    elseif (alpha == -beta)
        A = ((1 / size(I, 2)) * sum(((bsxfun(@rdivide, I .* (1 + B * C), C) + eps_nnz) .^ alpha) .* W, 2)) .^ (1 / alpha);
        A = max(A, 0);
        
        C = C - 0.5 * (sum(((1 - (eps_nnz + (I .* (1 + B * C)) ./ (A * C)) .^ alpha) ./ (eps_nnz + bsxfun(@times, C, 1 + B * C))) .* W, 1) + alpha_C * C ./ quantile(I, 0.9))./ ...
            (sum((((((I .* (1 + B * C)) ./ (A * C) + eps_nnz) .^ alpha) .* (2 * B * C + alpha + 1) - 2 * B * C - 1) ./ ...
            bsxfun(@times, C .^ 2, (1 + B * C) .^ 2)) .* W, 1) + alpha_C ./ (2 * quantile(I, 0.9)));
        C = max(C, 0);
        
        B = B - 0.5 * (sum((((eps_nnz + bsxfun(@rdivide, I, A)) .^ alpha) .* ((eps_nnz + bsxfun(@rdivide, 1 + B * C, C)) .^ (alpha - 1)) - ...
            bsxfun(@rdivide, C, 1 + B * C)) .* W, 2) + alpha_B * B ./ quantile(I', 0.9)') ./ ...
            (sum(((alpha - 1) * (((eps_nnz + bsxfun(@rdivide, I, A)) .^ alpha) .* ((eps_nnz + bsxfun(@rdivide, 1 + B * C, C)) .^ (alpha - 2))) + ...
            bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* W, 2) + alpha_B ./ (2 * quantile(I', 0.9)'));
        B = max(B, 0);
    else
        % optimizing A
        ...
        % optimizing C
        F = (A * C) ./ (1 + B * C);
        C = C - 0.5 * (sum((1 ./ (eps_nnz + A * (C .^ 2))) .* ((F + eps_nnz) .^ (beta + 1)) .* ((I + eps_nnz) .^ alpha - (F + eps_nnz) .^ alpha) .* W, 1) + alpha_C * C) ./ ...
            (sum((1 ./ (eps_nnz + bsxfun(@times, C .^ 2, (1 + B * C) .^ 2))) .* ((F + eps_nnz) .^ beta) .* (((F + eps_nnz) .^ alpha) .* (2 * B * C - alpha - beta + 1) - ...
            ((I + eps_nnz) .^ alpha) .* (2 * B * C - beta + 1)) .* W, 1) + alpha_C/2);
        C = max(C, 0);
        
        % optimizing B
        F = (A * C) ./ (1 + B * C);
        B = B + 0.5 * ((1 ./ (A + eps_nnz)) .* sum(((F + eps_nnz) .^ (beta + 1)) .* ((I + eps_nnz) .^ alpha - (F + eps_nnz) .^ alpha) .* W, 2) + alpha_B * B) ./ ...
            (sum(bsxfun(@rdivide, C .^ 2, (1 + B * C) .^ 2) .* ((F + eps_nnz) .^ beta) .* ((beta + 1) * ((I + eps_nnz) .^ alpha) - ...
            (alpha + beta + 1) * ((F + eps_nnz) .^ alpha)) .* W, 2) + alpha_B/2);
        B = max(B, 0);
    end
