...
tmp=[];
inten_sliced = inten_full_sliced(probesets);
inten_idx = inten_full_idx(probesets);
for i = 1:length(probesets)
    inten_sliced{i} = inten_full_sliced{probesets(i)}(:, arrays) + 1;
    tmp = [tmp; inten_idx{i}];
end
...
for ind = 1:length(alpha_range)*length(beta_range)
    ...
    goodness_of_fit_fro(i, j) = sum(sum(R.^2)) / length(arrays);
    goodness_of_fit_l1(i, j)  = sum(sum(abs(R))) / length(arrays);
    goodness_of_fit_weighted(i, j) = sum(sum(loss_asymmetric(R) .* inten(:, 1:end-2) )) / length(arrays);
