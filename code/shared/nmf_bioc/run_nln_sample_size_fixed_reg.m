...
for i = 1:length(train_sizes)
    ...
    for j = 1:50
        inten_train_sliced{j} = inten_train_big_sliced{j}(:, 1:train_size);
        inten_train(inten_full_idx{j}, :) = inten_train_big(inten_full_idx{j}, [1:train_size end-1 end]);
    end
    ...
    save('sample_size_frankenshtein_fixed_reg_m5', 'train_error', 'test_error', 'validation_error', ...
