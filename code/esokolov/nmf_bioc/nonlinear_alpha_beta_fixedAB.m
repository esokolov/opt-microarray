...
function [C isConverged] = nonlinear_alpha_beta_fixedAB(I, A, B, alpha, beta, maxIterCnt, eps, alpha_C, use_term_criteria)
    ...
    for currIter = 1:maxIterCnt
        if (alpha == 0 && beta == 0)
            F = (A * C) ./ (1 + B * C);
            C = C + 0.5 * sum((1 ./ (eps + bsxfun(@plus, C, B * (C .^ 2)))) .* (log(I + eps) - log(F + eps)), 1) ./ ...
                sum((1 ./ (eps + bsxfun(@times, C .^ 2, (1 + B * C) .^ 2))) .* ((2 * B * C + 1) .* log((I + eps) ./ (F + eps)) + 1), 1);            
            C = max(C, 0);
        elseif (alpha == 0)
            F = (A * C) ./ (1 + B * C);
            C = C - 0.5 * sum(((F + eps) .^ (beta + 1)) .* log((F + eps) ./ (I + eps)) .* (1 ./ (eps + A * (C .^ 2))), 1) ./ ...
                sum((1 ./ (eps + bsxfun(@times, C .^ 2, (1 + B * C) .^ 2))) .* ((F + eps) .^ beta) .* ...
                (1 - (2 * B * C - beta + 1) .* log((F + eps) ./ (I + eps))), 1);
            C = max(C, 0);
        elseif (beta == 0)
            C = C - 0.5 * sum(((((A * C) ./ (1 + B * C)) .^ alpha) - I .^ alpha) ./ ...
                bsxfun(@times, C, 1 + B * C), 1) ./ ...
                sum(((alpha - 1 - 2 * B * C) .* (((A * C) ./ (1 + B * C)) .^ alpha) + ...
                (1 + 2 * B * C) .* (I .^ alpha)) ./ ...
                bsxfun(@times, C .^ 2, (1 + B * C) .^ 2), 1);
            C = max(C, 0);
        elseif (alpha == -beta)
            C = C - 0.5 * sum((1 - ((I .* (1 + B * C)) ./ (A * C)) .^ alpha) ./ bsxfun(@times, C, 1 + B * C), 1) ./ ...
