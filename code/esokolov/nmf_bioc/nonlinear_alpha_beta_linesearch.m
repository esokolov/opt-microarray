function [A B C isConverged qual_hist C_max_hist B_max_hist A_max_hist] = nonlinear_alpha_beta_linesearch(I, alpha, beta, maxIterCnt, eps, ...
    ...
    for currIter = 1:maxIterCnt
        ...
        elseif (alpha == -beta)
            ...
            direction = - ((1 / alpha) * sum((1 - (eps + (I .* (1 + B * C)) ./ (A * C)) .^ alpha) ./ (eps + bsxfun(@times, C, 1 + B * C)), 1) + alpha_C * C)./ ...
            ...
            direction = - ((1 / alpha) * sum(((eps + bsxfun(@rdivide, I, A)) .^ alpha) .* ((eps + bsxfun(@rdivide, 1 + B * C, C)) .^ (alpha - 1)) - ...
