function [A B C isConverged qual_hist C_max_hist B_max_hist A_max_hist corr_B_hist test_qual_hist time iter_cnt] = nonlinear_alpha_beta_CQBWQ_4pics(I, alpha, beta, maxIterCnt, eps, ...
        alpha_B, alpha_C, use_term_criteria, I_test)
    if (nargin < 8)
    ...
    for currIter = 1:maxIterCnt
        if (alpha == 0 && beta == 0)
            ...
            A = A - ((1 ./ A) .* sum(log((F + eps_nnz) ./ (I + eps_nnz)), 2)) ./ ...
                ((1 ./ (A .^ 2)) .* (sum(log((I + eps_nnz) ./ (F + eps_nnz)), 2) + size(I, 2)));
            A = max(A, eps_nnz);
            ...
            F = (A * C) ./ (1 + B * C);
            B = B - (sum(bsxfun(@rdivide, C, 1 + B * C) .* log((I + eps_nnz) ./ (F + eps_nnz)), 2) + alpha_B * maxInten ./IQ .* B) ./ ...
                (sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* (log((F + eps_nnz) ./ (I + eps_nnz)) + 1), 2) + alpha_B * maxInten ./IQ);
            B = max(B, 0);
        elseif (alpha == 0)
            F = (A * C) ./ (1 + B * C);
            A = A - ((1 ./ A) .* sum(power_my(F + eps_nnz, beta) .* log((F + eps_nnz) ./ (I + eps_nnz)), 2)) ./ ...
                ((1 ./ (A .^ 2)) .* sum(power_my(F, beta) .* (1 + (beta - 1) * log((F + eps_nnz) ./ (I + eps_nnz))), 2));
            A = max(A, eps_nnz);
            ...
            F = (A * C) ./ (1 + B * C);
            B = B + (alpha_B * maxInten ./IQ .* B + (1 ./ A) .* sum(power_my(F, beta + 1) .* log((F + eps_nnz) ./ (I + eps_nnz)), 2)) ./ ...
                (sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* power_my(F + eps_nnz, beta) .* (1 + (beta + 1) * log((F + eps_nnz) ./ (I + eps_nnz))), 2) + alpha_B * maxInten ./IQ);
            B = max(B, 0);
        elseif (beta == 0)  % OK
        ...
        elseif (alpha == -beta)     % OK
            ...
            % optimizing B
            F = (A * C) ./ (1 + B * C);
            B = B - ((1 / alpha) * sum(bsxfun(@rdivide, C, 1 + B * C) .* power_my(F, beta) .* (power_my(I + eps_nnz, alpha) - power_my(F, alpha)), 2) + alpha_B * maxInten ./IQ .* B) ./ ...
                ((1 / alpha) * sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* ...
                power_my(F, beta) .* ((alpha + beta + 1) * power_my(F, alpha) - (beta + 1) * (power_my(I + eps_nnz, alpha))), 2) + alpha_B * maxInten ./IQ);
...
%         end
        ...
        currQuality_reg = nmf_alpha_beta_divergence(I, Q, alpha, beta) + 0.5 * alpha_B * maxInten * sum(B .^ 2 ./IQ) + 0.5 * alpha_C * sum(C .^ 2);
        ...
        %fprintf('%d: %f\t%f\t%e\t%e\n', currIter, currQuality, (currQuality_reg-currQuality)/currQuality*100,  max(C), max(B));
        %fprintf('%d: %e\n', currIter, C(912));
