function [A B C isConverged qual_hist C_max_hist B_max_hist A_max_hist corr_B_hist test_qual_hist] = nonlinear_alpha_beta_reg_derivative(I, alpha, beta, maxIterCnt, eps, ...
    ...
    for currIter = 1:maxIterCnt
        ...
        elseif (alpha == 0)
            F = (A * C) ./ (1 + B * C);
            A = A - (beta^2 * (1 ./ (A + eps_nnz)) .* sum(((F + eps_nnz) .^ beta) .* log((F + eps_nnz) ./ (I + epeps_nnzs)), 2)) ./ ...
                (beta^2 * (1 ./ (eps_nnz + A .^ 2)) .* sum(((F + eps_nnz) .^ beta) .* ((beta - 1) * log((F + eps_nnz) ./ (I + eps_nnz)) + 1), 2));
            A = max(A, eps_nnz);
            
            F = (A * C) ./ (1 + B * C);
            C = C - (beta^2 * sum(((F + eps_nnz) .^ (beta + 1)) .* log((F + eps_nnz) ./ (I + eps_nnz)) .* (1 ./ (eps_nnz + A * (C .^ 2))), 1) + ...
                reg_C_first(A, B, C)) ./ ...
                (beta^2 * sum((1 ./ (eps_nnz + bsxfun(@times, C .^ 2, (1 + B * C) .^ 2))) .* ((F + eps_nnz) .^ beta) .* ...
                (1 - (2 * B * C - beta + 1) .* log((F + eps_nnz) ./ (I + eps_nnz))), 1) + reg_C_second(A, B, C));
            C = max(C, 0);
            
            F = (A * C) ./ (1 + B * C);
            B = B +  (reg_B_first(A, B, C) + ...
                beta^2 * (1 ./ (A + eps_nnz)) .* sum(((F + eps_nnz) .^ (beta + 1)) .* log((F + eps_nnz) ./ (I + eps_nnz)), 2)) ./ ...
                (beta^2 * sum(bsxfun(@rdivide, C .^ 2, (1 + B * C) .^ 2) .* ((F + eps_nnz) .^ beta) .* ...
                ((beta + 1) * log((F + eps_nnz) ./ (I + eps_nnz)) + 1), 2) + reg_B_second(A, B, C));
            B = max(B, 0);
        elseif (beta == 0)
            F = (A * C) ./ (1 + B * C);
            A = A - ((1 ./ (alpha * A)) .* sum(F .^ alpha - I .^ alpha, 2)) ./ ...
                ((1 ./ (alpha * A .^ 2)) .* sum((alpha - 1) * (F .^ alpha) + I .^ alpha, 2));
            A = max(A, eps_nnz);
            
            C = C - ((1 / alpha) * sum((((eps_nnz + (A * C) ./ (1 + B * C)) .^ alpha) - (eps_nnz + I) .^ alpha) ./ ...
                bsxfun(@times, C, 1 + B * C), 1) + reg_C_first(A, B, C)) ./ ...
                ((1 / alpha) * sum(((alpha - 1 - 2 * B * C) .* ((eps_nnz + (A * C) ./ (1 + B * C)) .^ alpha) + ...
                (1 + 2 * B * C) .* ((I + eps_nnz) .^ alpha)) ./ ...
                bsxfun(@times, C .^ 2, (1 + B * C) .^ 2), 1) + reg_C_second(A, B, C));
            C = max(C, 0);
            
            B = B -  ((1 / alpha) * sum(bsxfun(@times, C, (I + eps_nnz) .^ alpha - (eps_nnz + (A * C) ./ (1 + B * C)) .^ alpha) ./ ...
                (1 + B * C), 2) + reg_B_first(A, B, C)) ./ ...
                ((1 / alpha) * sum(bsxfun(@times, C .^ 2, (alpha + 1) * (eps_nnz + (A * C) ./ (1 + B * C)) .^ alpha - (I + eps_nnz) .^ alpha) ./ ...
                ((1 + B * C) .^ 2), 2) + reg_B_second(A, B, C));
            B = max(B, 0);
        elseif (alpha == -beta)
            direction = -(size(I, 2) ./ (alpha * A) - ...
                (1 ./ (alpha * A .^ (alpha + 1))) .* sum(bsxfun(@rdivide, I .* (1 + B * C), C) .^ alpha, 2)) ./ ...
                (- size(I, 2) ./ (alpha * (A .^ 2)) + ((alpha + 1) ./ (alpha * (A .^ (alpha + 2)))) .* ...
                sum(bsxfun(@rdivide, I .* (1 + B * C), C) .^ alpha, 2));
            A = A + direction;
            A = max(A, eps_nnz);
            
            direction = - 0.5 * ((1 / alpha) * sum((1 - (eps_nnz + (I .* (1 + B * C)) ./ (A * C)) .^ alpha) ./ (eps_nnz + bsxfun(@times, C, 1 + B * C)), 1) + ...
                reg_C_first(A, B, C))./ ...
                ((1 / alpha) * sum(((((I .* (1 + B * C)) ./ (A * C) + eps_nnz) .^ alpha) .* (2 * B * C + alpha + 1) - 2 * B * C - 1) ./ ...
                bsxfun(@times, C .^ 2, (1 + B * C) .^ 2), 1) + reg_C_second(A, B, C));
            C = C + direction;
            C = max(C, 0);
            
            direction = - 0.5 * ((1 / alpha) * sum(((eps_nnz + bsxfun(@rdivide, I, A)) .^ alpha) .* ((eps_nnz + bsxfun(@rdivide, 1 + B * C, C)) .^ (alpha - 1)) - ...
                bsxfun(@rdivide, C, 1 + B * C), 2) + reg_B_first(A, B, C)) ./ ...
                ((1 / alpha) * sum((alpha - 1) * (((eps_nnz + bsxfun(@rdivide, I, A)) .^ alpha) .* ((eps_nnz + bsxfun(@rdivide, 1 + B * C, C)) .^ (alpha - 2))) + ...
...
%                ((I + eps) .^ alpha) .* (2 * B * C - beta + 1)), 1));
            C = C - ((1 ./ (alpha * C .^ 2)) .* sum(bsxfun(@times, 1 ./ A, (F .^ (beta + 1)) .* (F .^ alpha - (I + eps_nnz) .^ alpha)), 1) + ...
                reg_C_first(A, B, C)) ./ ...
                ((1 ./ (alpha * C .^ 2)) .* sum((1 ./ ((1 + B * C) .^ 2)) .* (F .^ beta) .* ((F .^ alpha) .* (alpha - 2 * B * C + beta - 1) + ...
                ((I + eps_nnz) .^ alpha) .* (2 * B * C - beta + 1)), 1) + ...
                reg_C_second(A, B, C));
            C = max(C, 0);

            % optimizing B
            F = (A * C) ./ (1 + B * C);
            B = B - ((1 / alpha) * sum(bsxfun(@rdivide, C, 1 + B * C) .* (F .^ beta) .* ((I + eps_nnz) .^ alpha - F .^ alpha), 2) + ...
                reg_B_first(A, B, C)) ./ ...
                ((1 / alpha) * sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* ...
                (F .^ beta) .* ((alpha + beta + 1) * (F .^ alpha) - (beta + 1) * ((I + eps_nnz) .^ alpha)), 2) + ...
                reg_B_second(A, B, C));
