function [A B C isConverged qual_hist C_max_hist B_max_hist A_max_hist corr_B_hist test_qual_hist] = nonlinear_alpha_beta_reg_derivative(I, alpha, beta, maxIterCnt, eps, ...
    ...
    for currIter = 1:maxIterCnt
        ...
        elseif (alpha == -beta)
            ...
            F = (A * C) ./ (1 + B * C);
            A = A - ((1/alpha) * (1 ./ (A + eps)) .* sum((F .^ beta) .* (F .^ alpha - (I + eps) .^ alpha), 2) - ...
                alpha_A * sum(bsxfun(@rdivide, 1 + B * C, A) .^ 2, 2)) ./ ...
                ((1 / alpha) * ((1 ./ (A + eps)) .^ 2) .* ...
                sum((F .^ beta) .* ((alpha + beta - 1) * (F .^ alpha) - (beta - 1) * ((I + eps) .^ alpha)), 2) + ...
                2 * alpha_A * sum(bsxfun(@rdivide, (1 + B * C) .^ 2, A .^ 3), 2));
            A = max(A, 1e-12);
            ...
            C = C - ((1 ./ (alpha * C .^ 2)) .* sum(bsxfun(@times, 1 ./ A, (F .^ (beta + 1)) .* (F .^ alpha - (I + eps) .^ alpha)), 1) + ...
                2 * alpha_C * sum(B ./ A, 1)) ./ ...
                ((1 ./ (alpha * C .^ 2)) .* sum((1 ./ ((1 + B * C) .^ 2)) .* (F .^ beta) .* ((F .^ alpha) .* (alpha - 2 * B * C + beta - 1) + ...
                ((I + eps) .^ alpha) .* (2 * B * C - beta + 1)), 1) + ...
                2 * alpha_C * sum((B .^ 2) ./ A, 1));
            C = max(C, 1e-12);

            % optimizing B
            F = (A * C) ./ (1 + B * C);
            B = B - ((1 / alpha) * sum(bsxfun(@rdivide, C, 1 + B * C) .* (F .^ beta) .* ((I + eps) .^ alpha - F .^ alpha), 2) + ...
                2 * alpha_B * sum(bsxfun(@rdivide, C, A) .* (1 + B * C), 2)) ./ ...
                ((1 / alpha) * sum((bsxfun(@rdivide, C, 1 + B * C) .^ 2) .* ...
                (F .^ beta) .* ((alpha + beta + 1) * (F .^ alpha) - (beta + 1) * ((I + eps) .^ alpha)), 2) + ...
                2 * alpha_B ./ A);
        ...
        currQuality_reg = nmf_alpha_beta_divergence(I, Q, alpha, beta) + alpha_A * sum(sum(bsxfun(@rdivide, (1 + B * C) .^ 2, A)));
